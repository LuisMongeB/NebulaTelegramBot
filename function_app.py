...
def test_function(req: func.HttpRequest) -> func.HttpResponse:
    ...
    return func.HttpResponse("Wow this first HTTP Function works!!!!", status_code=200)
...
@app.blob_trigger(
    arg_name="myblob", path="newcontainer/{name}.csv", connection="AdditionalStorage"
)
def test_function(myblob: func.InputStream):
    logging.info(
        f"Python blob Function triggered after the People.csv file was uploaded to the newcontainer. So cool!!!! \n"
        f"Printing the name of the blob path: {myblob.name}"
        f"Printing myblob type: {type(myblob)}"
        f"Printing the blob content: {myblob}"
    )
